...
const socket = io(serverUrl, connectionOption);
...
useEffect(() => {
  ...
  return (
    <>
      <ToastContainer />
      <Routes>
        <Route path="/" element={<JoinCreateRoom socket={socket} setUser={setUser} />} />
        <Route path="/room/:roomId" element={<Room user={user} socket={socket} users={users} />} />
      </Routes>
    </>
  );
}
